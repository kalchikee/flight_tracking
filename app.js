...
// ------------------------------------
ec2ServerURL = 'http://107.22.14.102'
...
async function showFlights() {
    ...
    try {
        const flightURL = ec2ServerURL + ':8000/flights/' + defaultAirportCode
        ...
        for (let i = 0; i < json.response.length;i++) {
            ...
            const popupContent = `
                <div style="font-family: Arial, sans-serif; min-width: 200px;">
                    <h4 style="margin: 0 0 10px 0; color: #333;">${json.response[i].flight_iata || json.response[i].flight_icao || 'Unknown Flight'}</h4>
                    <p style="margin: 5px 0;"><strong>Registration:</strong> ${json.response[i].reg_number || 'N/A'}</p>
                    <p style="margin: 5px 0;"><strong>Aircraft:</strong> ${json.response[i].aircraft_icao || 'N/A'}</p>
                    <p style="margin: 5px 0;"><strong>Airline:</strong> ${json.response[i].airline_iata || json.response[i].airline_icao || 'N/A'}</p>
                    <p style="margin: 5px 0;"><strong>From:</strong> ${json.response[i].dep_iata || 'N/A'}</p>
                    <p style="margin: 5px 0;"><strong>To:</strong> ${json.response[i].arr_iata || 'N/A'}</p>
                    <p style="margin: 5px 0;"><strong>Altitude:</strong> ${json.response[i].alt || 'N/A'} ft</p>
                    <p style="margin: 5px 0;"><strong>Speed:</strong> ${json.response[i].speed || 'N/A'} kts</p>
                    <p style="margin: 5px 0;"><strong>Direction:</strong> ${json.response[i].dir || 'N/A'}°</p>
                    <p style="margin: 5px 0;"><strong>Status:</strong> ${json.response[i].status || 'N/A'}</p>
                </div>
            `;

            // const marker = L.marker([json.response[i].lat,json.response[i].lng]).addTo(map)
            const marker = L.marker([json.response[i].lat,json.response[i].lng],{icon:planeIcon})
                .addTo(map)
