...
// ---------------------------------------------------
serverURL = "http://107.22.14.102:8000"
...
const getFlights = async function (airportCode)  {
    ...
    try {
        ...
        if (json && json.response && json.response.length > 0) {
            ...
            console.log("app.js: getFlights() No flights found or invalid response structure")
            // Create empty table entry
            populateFlightTable({response: []})
        }
...
const getNearbyAirports = async function ()  {
    ...
    try {
        ...
        // Check if response exists and has airports data
        if (json && json.response && json.response.airports && json.response.airports.length > 0) {
            ...
            console.log("app.js: getNearbyAirports() No nearby airports found or invalid response structure")
            // Handle no airports case
            const noAirportsResponse = {
                response: {
                    airports: []
                }
            }
            getNearbyAirportResults(noAirportsResponse)
        }
...
const populateFlightTable = (json) => {
    ...
    var tableCell
            ...
            // Indicate no data is available when API is not reachable or no flights
            if (!json || !json.response || json.response.length < 1) {

                // Create entry to indicate 'no data' available
                console.log("creating new table row for no data...")
                const tableRow = document.createElement('tr')

                tableCell = document.createElement("td")
                tableCell.setAttribute('colspan', '5')
                tableCell.append('No flight data available')
            ...
            for (let i = 0; i < json.response.length;i++) {
                ...
                const flightDetails = {
                    ...
                    // --------
                    tableCell = document.createElement("td")
                    tableCell.append(json.response[i].alt || 'N/A')
                    tableRow.append(tableCell)

                    // Aircraft Registration #
                    // -----------------------
                    tableCell = document.createElement('td')
                    tableCell.append(json.response[i].reg_number || 'N/A')
                    ...
                    // ---------
                    tableCell = document.createElement("td")
                    tableCell.append(json.response[i].dir || 'N/A')
                    tableRow.append(tableCell)

                    // Latitude
                    // ---------
                    tableCell = document.createElement("td")
                    tableCell.append(json.response[i].lat || 'N/A')
                    tableRow.append(tableCell)

                    // Longitude
                    // ---------
                    tableCell = document.createElement("td")
                    tableCell.append(json.response[i].lng || 'N/A')
                    tableRow.append(tableCell)
...
const getNearbyAirportResults = (json) => {
    ...
    try {
        ...
        if (!json || !json.response || !json.response.airports || json.response.airports.length < 1) {
            console.log("No nearby airport found from flight tracking API")

            airportName = 'No nearby airport found'
            airportIataCode = 'N/A'
            ...
            var airportName, airportIataCode;
    ...
    console.error("Error processing nearby airport data: " + error)

    airportName = 'Error getting nearby airport'
    airportIataCode = 'N/A'
