...
if (!myFlightsAPIKey || myFlightsAPIKey === "Your flight API Key here") {
  console.error("WARNING: Flight API key not configured properly in .env file!")
  console.error("Please update the flightsAPIKey in your .env file with a valid AirLabs API key")
}
...
app.use(function (req, res, next) {
  ...
  // +++++++++++++++++++++++++++++++++++++++++++++++
  const allowedOrigins = ['http://107.22.14.102']
...
app.get('/flights/:airport_code', async (request, response) => {
  ...
  try {
          ...
          if (!myFlightsAPIKey || myFlightsAPIKey === "Your flight API Key here") {
            console.error(scriptName + " API key not configured properly")
            response.json({
          ...
          if (!fetch_response.ok) {
            console.error(scriptName + " API request failed with status: " + fetch_response.status)
            response.json({
...
app.get('/nearbyAirports/:latitude,:longitude', async (request, response) => {
  ...
  try {
        ...
        // Check if API key is configured
        if (!myFlightsAPIKey || myFlightsAPIKey === "Your flight API Key here") {
          console.error("nearbyAirports: API key not configured properly")
          response.json({
            error: "API key not configured",
            response: { airports: [] }
          });
        ...
        if (!fetch_response.ok) {
          console.error("nearbyAirports: API request failed with status: " + fetch_response.status)
          response.json({
            error: "API request failed",
            response: { airports: [] }
          });
